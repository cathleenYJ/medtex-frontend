...
export const SignIn: React.FC<SignInProps> = ({ onDismiss }) => {
  ...
  const handleKeydown = (e: React.KeyboardEvent) => e.key === "Enter" && handleLogin();
  useEffect(() => {
    checkAuth();
  }, [pathname]);
  return (
    <div className="w-full max-w-md px-4 text-white">
      <form>
        <Field className="my-3">
          ...
          <Input className={clsx("mt-3 block w-full rounded-lg bg-gray-200/20 py-1.5 px-3 text-sm/6", "data-[focus]:outline-blue-500", isPending && "cursor-not-allowed")} onChange={handleChange} name="email" value={account.email} onKeyDown={handleKeydown} />
        </Field>
        <Field className="my-3">
          <Label className="text-sm/6 font-medium">Password</Label>
          <Input className={clsx("mt-3 block w-full rounded-lg bg-gray-200/20 py-1.5 px-3 text-sm/6", "data-[focus]:outline-blue-500", isPending && "cursor-not-allowed")} onChange={handleChange} name="password" value={account.password} onKeyDown={handleKeydown} />
        </Field>
