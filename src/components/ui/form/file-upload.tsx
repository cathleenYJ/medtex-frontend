...
export const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleUploadedFile = async (
    ...
    if (!new RegExp(`${type}/`).test(file.type))
      return openModal("Invalid file type");
    ...
    <fieldset>
      ...
      <label>
        <div
          tabIndex={0}
          className={clsx(
            "relative mt-2 block rounded-lg border-none bg-black/5 px-3 py-1.5 text-sm/6 text-black",
            "focus:not-data-focus:outline-none data-focus:outline-2 data-focus:-outline-offset-2 data-focus:outline-black/25",
            "w-3xs cursor-pointer flex justify-center items-center",
            error && "outline-2 outline-red-500"
          )}
        >
          {preview ? (
            <Image fill className="!relative" src={preview} alt="preview" />
          ) : (
